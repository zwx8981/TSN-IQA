...
def CL_Metric(srcc_matrix):
    ...
    for t in range(0, T):
        ...
        #plasticity = (plasticity + 1) / 2 + 1e-8
        #plasticity = np_sigmoid(plasticity)
        #plasticity = minmax_norm(plasticity, np_sigmoid(-1), max=np_sigmoid(1))

        plasticity = minmax_norm(plasticity, -1, 1)
        ...
        if t == 0:
            ...
            for s in range(0, t):
                #p1 = np_sigmoid(srcc_matrix[t, s])
                #p2 = np_sigmoid(srcc_matrix[s, s]) + eps
                #p1 = minmax_norm(p1, min=np_sigmoid(-1), max=np_sigmoid(1))
                #p2 = minmax_norm(p2, min=np_sigmoid(-1), max=np_sigmoid(1)) + 1e-8

                p1 = minmax_norm(srcc_matrix[t, s], -1, 1)
                p2 = minmax_norm(srcc_matrix[s, s], -1, 1) + eps
                PSR += (p1 / p2)
...
def parse_config():
    ...
    parser.add_argument("--backbone", type=str, default='vgg16')  # resnet34 or inceptionresnetv2
    parser.add_argument("--fc", type=bool, default=True)
    parser.add_argument('--scnn_root', type=str, default='saved_weights/scnn.pkl')

    parser.add_argument("--network", type=str, default="basecnn")  # basecnn or dbcnn or dbcnn2 or metaiqa or ma19 or koncept
    ...
    parser.add_argument('--ckpt_path', default='./checkpoint/bn_exp', type=str,
                        metavar='PATH', help='path to checkpoints')
    parser.add_argument('--ckpt_resume_path', default='./checkpoint/bn_exp', type=str,
                        metavar='PATH', help='path to checkpoints')
    ...
    parser.add_argument("--decay_interval", type=int, default=6)
    # parser.add_argument("--max_epochs", type=int, default=10)
    # parser.add_argument("--max_epochs2", type=int, default=20)
    # parser.add_argument("--lr", type=float, default=3e-4)
    # parser.add_argument("--decay_interval", type=int, default=16)
    ...
    parser.add_argument("--use_scnn", type=bool, default=True)

    parser.add_argument("--train_expert", type=bool, default=True)

    parser.add_argument("--experts_eval", type=bool, default=False)

    parser.add_argument("--meanfeat_val", type=bool, default=False)

    parser.add_argument("--replay", type=bool, default=False)
    parser.add_argument("--replay_memory", type=int, default=500)
    ...
    parser.add_argument("--double_distill", type=bool, default=False)

    parser.add_argument("--JL_CL", type=bool, default=False)

    parser.add_argument("--GDumb", type=bool, default=False)

    parser.add_argument("--new_replay", type=bool, default=False)
    ...
    #MAS
    parser.add_argument("--MAS", type=bool, default=True)

    #PiggyBack
    parser.add_argument("--piggyback", type=bool, default=False)
    ...
    parser.add_argument("--mask_decay", type=int, default=15)
    parser.add_argument("--class_decay", type=int, default=15)
    parser.add_argument("--threshold_fn", type=str, default='binarizer')

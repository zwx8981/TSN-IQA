...
import os
import numpy as np
from utils import CL_Metric, unified_metric
...
def TSN_IQA():
    for i in range(0, 1):
        for idx, task in enumerate(dataset):
            ...
            train_ewc = config.ewc
            train_lwf = config.lwf
            train_SI = config.SI
            train_MAS = config.MAS
            train_piggyback = config.piggyback
            config.dataset = dataset
            config.train_bn = True
            split = i + 1
            config.split = split
            #config.network = 'basecnn'

            #disable regularization when training fc layers
            config.lwf = False
            config.ewc = False
            config.SI = False
            config.MAS = False
            ...
            config.ckpt_path = os.path.join(config.base_ckpt_path, 'train_on_'+task)
            config.indicator = idx+1
            if idx > 0:
                config.ckpt_resume_path = os.path.join(config.base_ckpt_path, 'train_on_'+dataset[idx-1]) #resume from the previous task
                config.resume = True
                config.resume_best = True
                config.resume_new = True
            else:
            ...
            main(config)
            # stage2: fine-tuning the whole network
            config.ewc = train_ewc
            config.SI = train_SI
            config.MAS = train_MAS
            config.piggyback = train_piggyback
...
def eval_each_head():
    ...
    for i in range(0, 1):
        ...
        for idx, task in enumerate(dataset):
            ...
            if config.train_bn:  #hack
                config.subfolder = 'train_on_kadid10k'
                #config.enforce_bn = True
